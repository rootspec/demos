...
do_start() {
  ...
  sleep 2
  if pid_alive "$pid"; then
    # Read actual URL from log if available
    local actual_url
    actual_url=$(grep -oE 'https?://localhost:[0-9]+/?' "$LOG_FILE" 2>/dev/null | head -1)
    echo "Dev server started (PID $pid) at ${actual_url:-http://localhost:$resolved_port/}"
    echo "Logs: $LOG_FILE"
  else
